    ...
    <div class="table-container">
        ...
        >
            ...
            <template v-slot:status="{row}">
                <el-tag size="mini" :type="row.status | status('type')">{{row.status | status }}</el-tag>
            </template>
...
export default {
    ...
    mounted() {
        this.initColumns();
        this.$api.taskQuestionGroup.findAll().then(resp => {
            this.questionGroup = resp.data;
        });
        this.$api.messageGroup.findAll().then(resp => {
            this.messageGroup = resp.data;
        })
    },
    computed: {
    ...
    methods: {
        ...mapActions("taskProject", ["findPage", "findAll", "save", "delete"]),
        ...
        handleEdit: function(params) {
            let weekVal = params.row.weekday;
            this.weekday = JSON.parse(weekVal);
            ...
            this.$refs.editDataForm.validate(valid => {
                if (valid) {
                    this.$confirm("确认提交吗？", "提示", {}).then(() => {
                        ...
                        this.editDataForm.weekday = JSON.stringify(
                            this.weekday
                        );
        ...
        addQuestFunc: function() {
            ...
            api.editQuestion({
                ...
                if (!list) {
                    list = [];
                    Vue.set(
                        that.$store.state.editQuestionGroupForm,
                        "questionList",
                        list
                    );
                }
        ...
        removeQuestionFunc: function(listQuestion, index) {
            ...
            api.removeQuestion({
                ...
                that.$store.state.editQuestionGroupForm.questionList.splice(
                    index,
                    1
                );
            });
